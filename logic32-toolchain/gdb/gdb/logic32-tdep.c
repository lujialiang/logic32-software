/* Target-dependent code for logic32 CPU
...
#define LOGIC32_NUM_REGS        (34)

#define LOGIC32_REG_SIZE    (4)
...
/* Table of registers for LOGIC32.  This includes the hard registers
   and the soft registers used by GCC.  */
static char *
logic32_register_names[] =
{
...
static const char *
logic32_register_name (int reg_nr)
{
  if ((reg_nr <0)||(reg_nr>LOGIC32_NUM_REGS))
    return NULL;

  return logic32_register_names[reg_nr];
}

/* breakpoint is always the same as PC address w/0x00.  */
static const unsigned char *
logic32_breakpoint_from_pc (CORE_ADDR *pcptr, int *lenptr)
{
...
static struct type *
logic32_register_type (struct gdbarch *gdbarch, int reg_nr)
{
  return builtin_type_uint32;
}

static void
logic32_store_return_value (struct type *type, struct regcache *regcache,
                            const void *valbuf)
{
...
static void
logic32_extract_return_value (struct type *type, struct regcache *regcache,
                              void *valbuf)
{
...
logic32_return_value (struct gdbarch *gdbarch, struct type *valtype,
		      ...
		      const void *writebuf)
{
  if (TYPE_CODE (valtype) == TYPE_CODE_STRUCT
    ...
    {
      if (readbuf != NULL)
	logic32_extract_return_value (valtype, regcache, readbuf);
      if (writebuf != NULL)
	logic32_store_return_value (valtype, regcache, writebuf);
      return RETURN_VALUE_REGISTER_CONVENTION;
    }
}

static int
gdb_print_insn_logic32 (bfd_vma memaddr, disassemble_info *info)
{
	return print_insn_logic32 (memaddr, info);
}
...
CORE_ADDR logic32_skip_prologue (CORE_ADDR ip)
{
  struct logic32_unwind_cache tmp_cache = { 0 };

  return logic32_scan_prologue (ip, (CORE_ADDR) -1, &tmp_cache, NULL);
}

static CORE_ADDR
logic32_unwind_sp (struct gdbarch *gdbarch, struct frame_info *next_frame)
{
  ULONGEST sp;
...
static CORE_ADDR
logic32_unwind_pc (struct gdbarch *gdbarch, struct frame_info *next_frame)
{
  ULONGEST pc;
...
struct logic32_unwind_cache *
logic32_frame_unwind_cache (struct frame_info *next_frame,
                            void **this_prologue_cache)
{
  ...
  info = FRAME_OBSTACK_ZALLOC (struct logic32_unwind_cache);
  ...
  if (info->pc != 0)
    logic32_scan_prologue (info->pc, current_pc, info, info->saved_regs);
...
static void
logic32_frame_this_id (struct frame_info *next_frame,
                       void **this_prologue_cache,
                       struct frame_id *this_id)
{
  struct logic32_unwind_cache *info
    = logic32_frame_unwind_cache (next_frame, this_prologue_cache);
  CORE_ADDR func;
  struct frame_id id;
...
static void
logic32_frame_prev_register (struct frame_info *next_frame,
                             void **this_prologue_cache,
...
static const struct frame_unwind logic32_frame_unwind = {
  NORMAL_FRAME,
  logic32_frame_this_id,
  logic32_frame_prev_register
};

const struct frame_unwind *
logic32_frame_sniffer (struct frame_info *next_frame)
{
  return &logic32_frame_unwind;
}

static CORE_ADDR
logic32_frame_base_address (struct frame_info *next_frame, void **this_cache)
{
  struct logic32_unwind_cache *info
    = logic32_frame_unwind_cache (next_frame, this_cache);
...
static CORE_ADDR
logic32_frame_args_address (struct frame_info *next_frame, void **this_cache)
{
  CORE_ADDR addr;
  struct logic32_unwind_cache *info
    = logic32_frame_unwind_cache (next_frame, this_cache);
...
static const struct frame_base logic32_frame_base = {
  &logic32_frame_unwind,
  logic32_frame_base_address,
  logic32_frame_base_address,
  logic32_frame_args_address
};


static struct gdbarch *
logic32_gdbarch_init (struct gdbarch_info info,
                      struct gdbarch_list *arches)
{
  ...
  switch (info.bfd_arch_info->arch)
    {
    case bfd_arch_logic32:
      set_gdbarch_addr_bit (gdbarch, 32);
      set_gdbarch_pc_regnum (gdbarch, HARD_PC_REGNUM);
      set_gdbarch_num_regs (gdbarch, LOGIC32_NUM_REGS);
      break;
  ...
  set_gdbarch_skip_prologue (gdbarch, logic32_skip_prologue);
  set_gdbarch_write_pc (gdbarch, generic_target_write_pc);

	/* frame stuff */
  frame_unwind_append_sniffer (gdbarch, dwarf2_frame_sniffer);

  set_gdbarch_unwind_pc (gdbarch, logic32_unwind_pc);
  set_gdbarch_unwind_sp (gdbarch, logic32_unwind_sp);

	/* register stuff */
  set_gdbarch_sp_regnum (gdbarch, HARD_SP_REGNUM);
  set_gdbarch_register_name (gdbarch, logic32_register_name);
  set_gdbarch_register_type (gdbarch, logic32_register_type);

  set_gdbarch_return_value (gdbarch, logic32_return_value);
  set_gdbarch_inner_than (gdbarch, core_addr_lessthan);
  set_gdbarch_breakpoint_from_pc (gdbarch, logic32_breakpoint_from_pc);
  set_gdbarch_print_insn (gdbarch, gdb_print_insn_logic32);
  return gdbarch;
}

extern initialize_file_ftype _initialize_logic32_tdep; /* -Wmissing-prototypes */

void
_initialize_logic32_tdep (void)
{
  register_gdbarch_init (bfd_arch_logic32, logic32_gdbarch_init);
} 
